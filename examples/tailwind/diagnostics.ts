...
import { type Node, type NodeTag, parser as parse } from 'posthtml-parser'
...
export async function diagnose(files: string[]) {
  ...
  for (let file of files) {
    ...
    walk(tree, (node) => {
      let line = lines[(node.location?.start?.line ?? 0) - 1]
...
function location(row: number, col: number, len = 1): Location {
  return {
    ...
    end: { line: row, column: col + len - 1 },
  }
}

function walk(node: Node | Node[], cb: (node: NodeTag) => void) {
...
function splitClass(klass: string) {
...
function* extractClassNames(input: string) {
