...
function prepareSource(source: string) {
  return typeCode(rasterizeCode(source))
}
  ...
  let outputOfStrings = [
    ...
    ...output.map((row, i, all) => {
      ...
      let result = [
        ...
        // Start of note
        ...(rowType & Type.StartOfNote ? [styles.dim(CHARS.LConnector), styles.dim(CHARS.H)] : []),
...
export async function printer(
  ...
  let optimizedSources = new DefaultMap((file) => prepareSource(sources.get(file) ?? ''))

  // Report per block, that will be cleaner from a UI perspective
  write('') // Before
  for (let diagnostics of diagnosticsPerBlock) {
    await reportBlock(optimizedSources, diagnostics, write)
    write('')
  }
